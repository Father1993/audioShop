...
export const handleSelectPickupAddress = async (text: string) => {
  ...
  setShouldLoadMagnitolaData(true)

  const magnitolaData = await getMagnitolaOfficesByCityFx({
    city: text.split(' ')[0].replace(',', ''),
    lang: langFromLS,
  })

  return magnitolaData
}
...
//@ts-ignore
const fitToViewport = async (markerData, map) => {
...
export const handleResultClearing = (
  ...
  if (userGeolocation?.features) {
    ...
    map.setCenter([48.4261481667904, 135.10673314670237]).zoomTo(11)
  }
...
//@ts-ignore
export function initSearchMarket(ttMaps) {
  //@ts-ignore
